...
from config_ir import VertexAIConfig, GeminiModel


logger = logging.getLogger(__name__)
TARGET_SIZE =(1024, 1024)  #(2048, 1024) (3840, 1920) 
SYSTEM_PROMPT = """You are an expert AI in Computational Photography, Color Science, and Spectral Imaging.
Your specialty is simulating non-visible light photography (specifically Near-Infrared and False-Color Infrared) on standard RGB images.

When performing style transfer from Reference Images (Real NIR) to a Target Image (Standard RGB), you must follow these principles:
1. **Semantic Consistency**: Do not just copy textures. Understand that NIR effects depend on material properties. Chlorophyll (vegetation) reflects IR light strongly, appearing Pink, Red, or White. Water and Sky usually absorb IR.
2. **Physics Simulation**: Treat the transformation as a "Channel Swapping" or "Spectral Remapping" process (e.g., simulating a specific lens coating or 720nm filter).
3. **High Fidelity**: Maintain the structural integrity and resolution of the Target Image. Only alter the color relationships.
4. **Pattern Extraction (For Multiple References)**: If provided with multiple reference images, do not mimic the specific composition of any single image. Instead, **extract the consistent "Lens Signature"** (the shared color mapping logic) across all references and apply that unified logic to the Target.
"""
class VertexAIClient:
    ...
    def _load_image_RESIZED(self, image_path: Union[str, Path]) -> Part:
        """
        Load image from file path and convert to Vertex AI Part
    ...
    def generate_image(
        self,
        prompt: str,
        input_image_paths: Optional[Union[str, Path, List[Union[str, Path]]]] = None,
        reference_image_paths: Optional[Union[str, Path, List[Union[str, Path]]]] = None,
        ...
        Args:
            ...
            input_image_paths: Ordered image(s) sent to the model
            reference_image_paths: Optional reference image(s) for style guidance
        ...
        if input_image_paths:
            if isinstance(input_image_paths, (str, Path)):
                reference_list = [input_image_paths]
            else:
                reference_list = list(input_image_paths)
        elif reference_image_paths:
            if isinstance(reference_image_paths, (str, Path)):
                reference_list = [reference_image_paths]
            else:
                reference_list = list(reference_image_paths)
        else:
            reference_list = []

        for idx in range(number_of_images):
            content = [Part.from_text(SYSTEM_PROMPT)]
            for ref_path in reference_list:
                ref_image_part = self._load_image(ref_path)
